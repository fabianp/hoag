...
def hoag_lbfgs(
    ...
    for it in range(1, maxiter):
        ...
        while 1:
            ...
            if task_str.startswith(b'FG'):
                ...
                if n_iterations > maxiter_inner:
                    ...
                    if verbose > 1:
                        h_func, h_grad = h_func_grad(x, lambdak)
                        print('inner level iterations: %s, objective %s' % (n_iterations, h_func))
            else:
                if verbose > 0:
        ...
        Bxk, success = splinalg.cg(B_op, g_grad, x0=Bxk, tol=epsilon_tol, maxiter=maxiter_inner)
        if success is False:
            raise ValueError
